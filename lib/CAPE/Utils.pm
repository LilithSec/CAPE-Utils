...
is set, the submission command is prefixed with "sudo -u <cape_runas>";
otherwise it dies.
...
sub _cape_runas_prefix {
	...
	my $configured_user = $self->{'config'}->{'_'}->{'cape_runas'};
	my $current_user    = getpwuid($>);

	if ( defined($current_user) && $current_user eq $configured_user ) {
		return ();
	}

	if ( $self->{'config'}->{'_'}->{'enable_sudo'} ) {
		return ( 'sudo', '-u', $configured_user );
	}

	die(      'Not being ran as the configured user "'
			. $configured_user . '"'
			. ( defined($current_user) ? ', currently running as "' . $current_user . '"' : '' )
			. ' and enable_sudo is not set' );
} ## end sub _cape_runas_prefix
...
"poetry run" so it runs inside CAPEv2's poetry environment.
